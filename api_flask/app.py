...
def customer_controller(id_: int):
    id_ = int(id_)
...
def predict_controller(id_: int):
    id_ = int(id_)
...
# Si le "n_customers" est plus élevé que la taille maximale des données alors la requêtre renverra une erreur 500.
...
# Cette partie permet d'attraper les erreurs renvoyées à l'intérieur des différents controlleurs pour les convertir en
